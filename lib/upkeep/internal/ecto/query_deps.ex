defmodule Upkeep.Internal.Ecto.QueryDeps do
  @moduledoc false
  ...
  def from_query(%Ecto.Query{} = query) do
    bindings = Upkeep.Internal.Ecto.QueryDeps.Bindings.from_query(query)
  ...
  defp collect_query_fields(deps, query) do
    query
    |> Upkeep.Internal.Ecto.QueryDeps.Expressions.all()
  ...
  defp collect_fields(deps, expr) do
    {_expr, deps} =
      Macro.prewalk(expr, deps, fn
        node, deps ->
          case Upkeep.Internal.Ecto.QueryDeps.Expressions.field_ref(node) do
  ...
  defp maybe_add_equality_filter(filters, field_side, value_side, params) do
    with {binding, field} <- Upkeep.Internal.Ecto.QueryDeps.Expressions.field_ref(field_side),
  ...
  defp collect_subquery_deps(deps, query) do
    query
    |> Upkeep.Internal.Ecto.QueryDeps.Expressions.structs()
  ...
  defp preload_schema_reasons(query) do
    root_schema = Map.get(Upkeep.Internal.Ecto.QueryDeps.Bindings.from_query(query), 0)
